...
void add_expense() {
	if (expense_count >= MAX_EXPENSES) { cout << "Storage full.\n"; return; }
	...
	if (idx == 0) strcpy(e.category, "food");
	else if (idx == 1) strcpy(e.category, "travel");
	else if (idx == 2) strcpy(e.category, "shopping");
	else {
		cout << "Enter category name (max 19 chars): ";
		cin.getline(e.category, sizeof(e.category));
		if (e.category[0] == '\0') strcpy(e.category, "other");
	}
	...
	if (e.date[0] == '\0') strcpy(e.date, "-");

	cat_totals[idx] += e.amount;
	expense_count++;
	cout << "Added "; print_amount(e.amount); cout << " to " << e.category << " on " << e.date << "\n";
}

void show_totals() {
	double total = 0.0;
	for (int i = 0; i < 4; ++i) total += cat_totals[i];
	cout << "\nTotal spending: "; print_amount(total); cout << "\n";
	cout << "By category:\n";
	cout << "  food     : "; print_amount(cat_totals[0]); cout << "\n";
	cout << "  travel   : "; print_amount(cat_totals[1]); cout << "\n";
	cout << "  shopping : "; print_amount(cat_totals[2]); cout << "\n";
	cout << "  other    : "; print_amount(cat_totals[3]); cout << "\n\n";
}

void list_expenses() {
	cout << "\nAll expenses:\n";
	if (expense_count == 0) { cout << "  (none)\n\n"; return; }
	for (int i = 0; i < expense_count; ++i) {
		Expense &e = expenses[i];
		cout << i+1 << ") " << e.date << " " << e.category << " ";
		print_amount(e.amount);
		if (e.note[0] != '\0') cout << "  - " << e.note;
		cout << "\n";
	}
	cout << "\n";
}
...
int main() {
	...
	while (true) {
		...
		else if (choice == 1) add_expense();
		else if (choice == 2) show_totals();
		else if (choice == 3) list_expenses();
